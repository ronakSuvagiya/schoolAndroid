package com.apps.smartschoolmanagement.photoview.view;
...
import com.apps.smartschoolmanagement.log.LogManager;
import com.apps.smartschoolmanagement.photoview.gestures.OnGestureListener;
import com.apps.smartschoolmanagement.photoview.gestures.VersionedGestureDetector;
import com.apps.smartschoolmanagement.photoview.scrollerproxy.ScrollerProxy;
import java.lang.ref.WeakReference;

public class PhotoViewAttacher implements IPhotoView, OnTouchListener, OnGestureListener, OnGlobalLayoutListener {
    private static final boolean DEBUG = Log.isLoggable ("", 3);
    ...
    private com.apps.smartschoolmanagement.photoview.gestures.GestureDetector mScaleDragDetector;
    ...
    /* renamed from: com.apps.smartschoolmanagement.photoview.view.PhotoViewAttacher$1 */
    ...
    /* renamed from: com.apps.smartschoolmanagement.photoview.view.PhotoViewAttacher$2 */
    static /* synthetic */ class C13872 {
    ...
    private class FlingRunnable implements Runnable {
        ...
        public void cancelFling() {
            if (PhotoViewAttacher.DEBUG) {
                LogManager.getLogger().mo3961d(PhotoViewAttacher.LOG_TAG, "Cancel Fling");
            }
        ...
        public void fling(int viewWidth, int viewHeight, int velocityX, int velocityY) {
            ...
            if (rect != null) {
                ...
                if (PhotoViewAttacher.DEBUG) {
                    LogManager.getLogger().mo3961d(PhotoViewAttacher.LOG_TAG, "fling. StartX:" + startX + " StartY:" + startY + " MaxX:" + maxX + " MaxY:" + maxY);
                }
        ...
        public void run() {
            if (!this.mScroller.isFinished()) {
                ...
                if (imageView != null && this.mScroller.computeScrollOffset()) {
                    ...
                    if (PhotoViewAttacher.DEBUG) {
                        LogManager.getLogger().mo3961d(PhotoViewAttacher.LOG_TAG, "fling run(). CurrentX:" + this.mCurrentX + " CurrentY:" + this.mCurrentY + " NewX:" + newX + " NewY:" + newY);
                    }
    ...
    private static boolean isSupportedScaleType(ScaleType scaleType) {
        ...
        switch (C13872.$SwitchMap$android$widget$ImageView$ScaleType[scaleType.ordinal()]) {
            case 1:
    ...
    public PhotoViewAttacher(ImageView imageView, boolean zoomable) {
        ...
        if (!imageView.isInEditMode()) {
            ...
            this.mGestureDetector = new GestureDetector(imageView.getContext(), new C13861());
    ...
    public ImageView getImageView() {
        ...
        if (imageView == null) {
            cleanup();
            LogManager.getLogger().mo3965i(LOG_TAG, "ImageView no longer exists. You should not use this PhotoViewAttacher any more.");
        }
        return imageView;
    }
    ...
    public void onDrag(float dx, float dy) {
        if (!this.mScaleDragDetector.isScaling()) {
            if (DEBUG) {
                LogManager.getLogger().mo3961d(LOG_TAG, String.format("onDrag: dx: %.2f. dy: %.2f", new Object[]{Float.valueOf(dx), Float.valueOf(dy)}));
            }
            ImageView imageView = getImageView();
    ...
    public void onFling(float startX, float startY, float velocityX, float velocityY) {
        if (DEBUG) {
            LogManager.getLogger().mo3961d(LOG_TAG, "onFling. sX: " + startX + " sY: " + startY + " Vx: " + velocityX + " Vy: " + velocityY);
        }
        ImageView imageView = getImageView();
    ...
    public void onScale(float scaleFactor, float focusX, float focusY) {
        if (DEBUG) {
            LogManager.getLogger().mo3961d(LOG_TAG, String.format("onScale: scale: %.2f. fX: %.2f. fY: %.2f", new Object[]{Float.valueOf(scaleFactor), Float.valueOf(focusX), Float.valueOf(focusY)}));
        }
        ...
        switch (ev.getAction()) {
            case 0:
                if (parent != null) {
                    ...
                    LogManager.getLogger().mo3965i(LOG_TAG, "onTouch getParent() returned null");
                }
                cancelFling();
                break;
            case 1:
    ...
    public void setScale(float scale, float focalX, float focalY, boolean animate) {
        ...
        if (scale < this.mMinScale || scale > this.mMaxScale) {
            LogManager.getLogger().mo3965i(LOG_TAG, "Scale must be within the range of minScale and maxScale");
        } else if (animate) {
    ...
    private boolean checkMatrixBounds() {
        ...
        if (height <= ((float) viewHeight)) {
            switch (C13872.$SwitchMap$android$widget$ImageView$ScaleType[this.mScaleType.ordinal()]) {
                case 2:
                    deltaY = -rect.top;
                    break;
                case 3:
