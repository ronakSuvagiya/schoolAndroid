...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs) {
        ...
        try {
            this.mFlow = a.getBoolean(2, DEFAULT_FLOW);
            this.mChildSpacing = a.getInt(0, 0);
        } catch (NumberFormatException e) {
            this.mChildSpacing = a.getDimensionPixelSize(0, (int) dpToPx(0.0f));
        } catch (Throwable th) {
            a.recycle();
        }
        try {
            this.mChildSpacingForLastRow = a.getInt(1, -65538);
        } catch (NumberFormatException e2) {
            this.mChildSpacingForLastRow = a.getDimensionPixelSize(1, (int) dpToPx(0.0f));
        }
        try {
            this.mRowSpacing = (float) a.getInt(4, 0);
        } catch (NumberFormatException e3) {
            this.mRowSpacing = a.getDimension(4, dpToPx(0.0f));
        }
        this.mMaxRows = a.getInt(3, Integer.MAX_VALUE);
        this.mRtl = a.getBoolean(5, false);
        a.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        boolean allowFlow = (widthMode == 0 || !this.mFlow) ? false : DEFAULT_FLOW;
