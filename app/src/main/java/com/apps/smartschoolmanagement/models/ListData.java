package com.apps.smartschoolmanagement.models;

public class ListData {
    public String Library_bookName;
    public String Library_dateofsubmit;
    public String Library_dateoftaken;
    public String Library_status_text;
    public String Library_studentName;
    public String appointmentid;
    public String assignment_assignment;
    public String assignment_id;
    public String assignment_subject;
    public String assignment_submit_date;
    public String assignment_teacher;
    public String bus_driver_name;
    public String bus_latitude;
    public String bus_logitude;
    public String bus_phone_number;
    public String bus_route_name;
    public String bus_transport_id;
    public String classname;
    public String date;
    public String designation;
    public String exam_comment;
    public String exam_date;
    public String exam_id;
    public String exam_name;
    public String exam_semister;
    public String id;
    public String leave_date;
    public String leave_id;
    public String leave_reason;
    public String leave_student;
    public String leave_studentclass;
    public String leave_studentid;
    public String leave_teacher;
    public String leave_type;
    public String name;
    public String notify_date;
    public String notify_message;
    public String notify_time;
    public String notify_title;
    public String photo;
    public String regarding;
    public String remarks;
    public String remarks_date;
    public String remarks_student;
    public String remarks_teacher;
    public String remarks_type;
    public String roll;
    public String school;
    public String staff_address;
    public String staff_birthday;
    public String staff_bloodgroup;
    public String staff_class;
    public String staff_ctc;
    public String staff_email;
    public String staff_experience;
    public String staff_gender;
    public String staff_id;
    public String staff_joindate;
    public String staff_name;
    public String staff_phone;
    public String staff_photo;
    public String staff_religion;
    public String staff_subject;
    public String status;
    public String student_class;
    public String student_id;
    public String student_name;
    public String student_photo;
    public String student_roll;
    public String time;
    public String village;

    public String getStatus() {
        return this.status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getAppointmentid() {
        return this.appointmentid;
    }

    public void setAppointmentid(String appointmentid) {
        this.appointmentid = appointmentid;
    }

    public String getClassname() {
        return this.classname;
    }

    public void setClassname(String classname) {
        this.classname = classname;
    }

    public String getDate() {
        return this.date;
    }

    public void setDate(String date) {
        this.date = date;
    }

    public String getTime() {
        return this.time;
    }

    public void setTime(String time) {
        this.time = time;
    }

    public String getRegarding() {
        return this.regarding;
    }

    public void setRegarding(String regarding) {
        this.regarding = regarding;
    }

    public String getId() {
        return this.id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getRoll() {
        return this.roll;
    }

    public void setRoll(String roll) {
        this.roll = roll;
    }

    public String getPhoto() {
        return this.photo;
    }

    public void setPhoto(String photo) {
        this.photo = photo;
    }

    public String getStudent_photo() {
        return this.student_photo;
    }

    public void setStudent_photo(String student_photo) {
        this.student_photo = student_photo;
    }

    public String getStudent_id() {
        return this.student_id;
    }

    public void setStudent_id(String student_id) {
        this.student_id = student_id;
    }

    public String getStudent_roll() {
        return this.student_roll;
    }

    public void setStudent_roll(String student_roll) {
        this.student_roll = student_roll;
    }

    public String getStudent_class() {
        return this.student_class;
    }

    public void setStudent_class(String student_class) {
        this.student_class = student_class;
    }

    public String getStudent_name() {
        return this.student_name;
    }

    public void setStudent_name(String student_name) {
        this.student_name = student_name;
    }

    public String getStaff_class() {
        return this.staff_class;
    }

    public void setStaff_class(String staff_class) {
        this.staff_class = staff_class;
    }

    public String getStaff_ctc() {
        return this.staff_ctc;
    }

    public void setStaff_ctc(String staff_ctc) {
        this.staff_ctc = staff_ctc;
    }

    public String getStaff_name() {
        return this.staff_name;
    }

    public void setStaff_name(String staff_name) {
        this.staff_name = staff_name;
    }

    public String getStaff_email() {
        return this.staff_email;
    }

    public void setStaff_email(String staff_email) {
        this.staff_email = staff_email;
    }

    public String getStaff_phone() {
        return this.staff_phone;
    }

    public void setStaff_phone(String staff_phone) {
        this.staff_phone = staff_phone;
    }

    public String getStaff_gender() {
        return this.staff_gender;
    }

    public void setStaff_gender(String staff_gender) {
        this.staff_gender = staff_gender;
    }

    public String getStaff_birthday() {
        return this.staff_birthday;
    }

    public void setStaff_birthday(String staff_birthday) {
        this.staff_birthday = staff_birthday;
    }

    public String getStaff_religion() {
        return this.staff_religion;
    }

    public void setStaff_religion(String staff_religion) {
        this.staff_religion = staff_religion;
    }

    public String getStaff_bloodgroup() {
        return this.staff_bloodgroup;
    }

    public void setStaff_bloodgroup(String staff_bloodgroup) {
        this.staff_bloodgroup = staff_bloodgroup;
    }

    public String getStaff_address() {
        return this.staff_address;
    }

    public void setStaff_address(String staff_address) {
        this.staff_address = staff_address;
    }

    public String getStaff_subject() {
        return this.staff_subject;
    }

    public void setStaff_subject(String staff_subject) {
        this.staff_subject = staff_subject;
    }

    public String getStaff_id() {
        return this.staff_id;
    }

    public void setStaff_id(String staff_id) {
        this.staff_id = staff_id;
    }

    public String getStaff_photo() {
        return this.staff_photo;
    }

    public void setStaff_photo(String staff_photo) {
        this.staff_photo = staff_photo;
    }

    public String getStaff_experience() {
        return this.staff_experience;
    }

    public void setStaff_experience(String staff_experience) {
        this.staff_experience = staff_experience;
    }

    public String getStaff_joindate() {
        return this.staff_joindate;
    }

    public void setStaff_joindate(String staff_joindate) {
        this.staff_joindate = staff_joindate;
    }

    public String getLibrary_studentName() {
        return this.Library_studentName;
    }

    public void setLibrary_studentName(String library_studentName) {
        this.Library_studentName = library_studentName;
    }

    public String getLibrary_bookName() {
        return this.Library_bookName;
    }

    public void setLibrary_bookName(String library_bookName) {
        this.Library_bookName = library_bookName;
    }

    public String getLibrary_dateoftaken() {
        return this.Library_dateoftaken;
    }

    public void setLibrary_dateoftaken(String library_dateoftaken) {
        this.Library_dateoftaken = library_dateoftaken;
    }

    public String getLibrary_dateofsubmit() {
        return this.Library_dateofsubmit;
    }

    public void setLibrary_dateofsubmit(String library_dateofsubmit) {
        this.Library_dateofsubmit = library_dateofsubmit;
    }

    public String getLibrary_status_text() {
        return this.Library_status_text;
    }

    public void setLibrary_status_text(String library_status_text) {
        this.Library_status_text = library_status_text;
    }

    public String getBus_driver_name() {
        return this.bus_driver_name;
    }

    public void setBus_driver_name(String bus_driver_name) {
        this.bus_driver_name = bus_driver_name;
    }

    public String getBus_phone_number() {
        return this.bus_phone_number;
    }

    public void setBus_phone_number(String bus_phone_number) {
        this.bus_phone_number = bus_phone_number;
    }

    public String getBus_transport_id() {
        return this.bus_transport_id;
    }

    public void setBus_transport_id(String bus_transport_id) {
        this.bus_transport_id = bus_transport_id;
    }

    public String getBus_route_name() {
        return this.bus_route_name;
    }

    public void setBus_route_name(String bus_route_name) {
        this.bus_route_name = bus_route_name;
    }

    public String getBus_latitude() {
        return this.bus_latitude;
    }

    public void setBus_latitude(String bus_latitude) {
        this.bus_latitude = bus_latitude;
    }

    public String getBus_logitude() {
        return this.bus_logitude;
    }

    public void setBus_logitude(String bus_logitude) {
        this.bus_logitude = bus_logitude;
    }

    public String getLeave_id() {
        return this.leave_id;
    }

    public void setLeave_id(String leave_id) {
        this.leave_id = leave_id;
    }

    public String getLeave_teacher() {
        return this.leave_teacher;
    }

    public void setLeave_teacher(String leave_teacher) {
        this.leave_teacher = leave_teacher;
    }

    public String getLeave_student() {
        return this.leave_student;
    }

    public void setLeave_student(String leave_student) {
        this.leave_student = leave_student;
    }

    public String getLeave_studentid() {
        return this.leave_studentid;
    }

    public void setLeave_studentid(String leave_studentid) {
        this.leave_studentid = leave_studentid;
    }

    public String getLeave_studentclass() {
        return this.leave_studentclass;
    }

    public void setLeave_studentclass(String leave_studentclass) {
        this.leave_studentclass = leave_studentclass;
    }

    public String getLeave_date() {
        return this.leave_date;
    }

    public void setLeave_date(String leave_date) {
        this.leave_date = leave_date;
    }

    public String getLeave_reason() {
        return this.leave_reason;
    }

    public void setLeave_reason(String leave_reason) {
        this.leave_reason = leave_reason;
    }

    public String getAssignment_subject() {
        return this.assignment_subject;
    }

    public void setAssignment_subject(String assignment_subject) {
        this.assignment_subject = assignment_subject;
    }

    public String getAssignment_teacher() {
        return this.assignment_teacher;
    }

    public void setAssignment_teacher(String assignment_teacher) {
        this.assignment_teacher = assignment_teacher;
    }

    public String getAssignment_submit_date() {
        return this.assignment_submit_date;
    }

    public void setAssignment_submit_date(String assignment_submit_date) {
        this.assignment_submit_date = assignment_submit_date;
    }

    public String getAssignment_assignment() {
        return this.assignment_assignment;
    }

    public void setAssignment_assignment(String assignment_assignment) {
        this.assignment_assignment = assignment_assignment;
    }

    public String getAssignment_id() {
        return this.assignment_id;
    }

    public void setAssignment_id(String assignment_id) {
        this.assignment_id = assignment_id;
    }

    public String getExam_id() {
        return this.exam_id;
    }

    public void setExam_id(String exam_id) {
        this.exam_id = exam_id;
    }

    public String getExam_name() {
        return this.exam_name;
    }

    public void setExam_name(String exam_name) {
        this.exam_name = exam_name;
    }

    public String getExam_semister() {
        return this.exam_semister;
    }

    public void setExam_semister(String exam_semister) {
        this.exam_semister = exam_semister;
    }

    public String getExam_date() {
        return this.exam_date;
    }

    public void setExam_date(String exam_date) {
        this.exam_date = exam_date;
    }

    public String getExam_comment() {
        return this.exam_comment;
    }

    public void setExam_comment(String exam_comment) {
        this.exam_comment = exam_comment;
    }

    public String getRemarks_type() {
        return this.remarks_type;
    }

    public void setRemarks_type(String remarks_type) {
        this.remarks_type = remarks_type;
    }

    public String getRemarks_teacher() {
        return this.remarks_teacher;
    }

    public void setRemarks_teacher(String remarks_teacher) {
        this.remarks_teacher = remarks_teacher;
    }

    public String getRemarks_student() {
        return this.remarks_student;
    }

    public void setRemarks_student(String remarks_student) {
        this.remarks_student = remarks_student;
    }

    public String getRemarks() {
        return this.remarks;
    }

    public void setRemarks(String remarks) {
        this.remarks = remarks;
    }

    public String getRemarks_date() {
        return this.remarks_date;
    }

    public void setRemarks_date(String remarks_date) {
        this.remarks_date = remarks_date;
    }

    public String getNotify_title() {
        return this.notify_title;
    }

    public void setNotify_title(String notify_title) {
        this.notify_title = notify_title;
    }

    public String getNotify_message() {
        return this.notify_message;
    }

    public void setNotify_message(String notify_message) {
        this.notify_message = notify_message;
    }

    public String getNotify_date() {
        return this.notify_date;
    }

    public void setNotify_date(String notify_date) {
        this.notify_date = notify_date;
    }

    public String getNotify_time() {
        return this.notify_time;
    }

    public void setNotify_time(String notify_time) {
        this.notify_time = notify_time;
    }

    public String getName() {
        return this.name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getDesignation() {
        return this.designation;
    }

    public void setDesignation(String designation) {
        this.designation = designation;
    }

    public String getSchool() {
        return this.school;
    }

    public void setSchool(String school) {
        this.school = school;
    }

    public String getVillage() {
        return this.village;
    }

    public void setVillage(String village) {
        this.village = village;
    }

    public String getLeave_type() {
        return this.leave_type;
    }

    public void setLeave_type(String leave_type) {
        this.leave_type = leave_type;
    }
}
